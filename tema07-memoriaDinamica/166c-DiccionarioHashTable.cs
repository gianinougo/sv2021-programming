...
// Versión c: Hashtable, sin Generics
        ...
        static void Main()
        {
            ...
            do
            {
                ...
                switch (opcion)
                {
                    ...
                    case "3":
                        IDictionaryEnumerator enumerador = miDiccio.GetEnumerator();
                        while (enumerador.MoveNext())
                        {
                            Console.WriteLine("{0} = {1}",
                                ((DictionaryEntry) enumerador.Current).Key,
                                ((DictionaryEntry) enumerador.Current).Value);
                        }
                        break;
